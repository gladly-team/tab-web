...
const MothersDay2023 = ({ user }) => {
  ...
  const onOpen = () => {
    setOpenWidget(true)
    gtag('event', 'mothers_day_2023_open_promo')
  }

  const onClose = () => {
    setOpenWidget(false)
    gtag('event', 'mothers_day_2023_close_promo')
  }

  const onDismiss = (e) => {
    e.stopPropagation()
    e.nativeEvent.stopImmediatePropagation()
    gtag('event', 'mothers_day_2023_dismiss')
  ...
  useEffect(() => {
    ...
    const dissNotif =
      localStorageMgr.getItem(getNotifDismissKey('mothers-day-2023')) || false
