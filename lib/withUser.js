...
const withUserHOC = renderFunction => {
  const WithUserComp = props => {
    ...
    const ChildComponent = renderFunction(authUser)
    return (
      <ChildComponent
        {...otherProps}
        authUser={authUser}
  ...
  WithUserComp.getInitialProps = async ctx => {
    ...
    // TODO: normalize authUser structure for server and client
    ...
    // Evaluate the child component's getInitialProps().
    const ChildComponent = renderFunction(authUserFromSession)
    let composedInitialProps = {}
    if (ChildComponent.getInitialProps) {
      composedInitialProps = await ChildComponent.getInitialProps(ctx)
    }
    return {
  ...
  WithUserComp.displayName = `WithUser(ChildComponent)`
